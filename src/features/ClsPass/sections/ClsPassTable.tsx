...
const columns: {
  ...
  { key: 'price', title: '기본금액', className: 'min-w-[120px] text-center' },
  {
    key: 'paidAmt',
    title: '결제 금액',
  ...
  { key: 'totalCnt', title: '총 회차', className: 'min-w-[100px] text-center' },
  {
    key: 'remainCnt',
    title: '잔여 회차',
  ...
  },
  {
    ...
    className: 'min-w-[150px]',
    render: (value) =>
      String(value).substring(0, 4) + '-' + String(value).substring(4, 6) + '-' + String(value).substring(6, 8),
  },
  {
    key: 'refundYn',
    title: '환불여부',
...
const formatNumber = (n: unknown) => new Intl.NumberFormat('ko-KR', { maximumFractionDigits: 0 }).format(toNumber(n));
...
export default function ClsPassTable({ data, isLoading, onDetailView }: ClsPassProps) {
  ...
  if (data.length === 0) {
    return (
      <div className="h-full w-full flex justify-center items-center font-medium text-gray">
        조회된 데이터가 없습니다
      </div>
    );
  }
    ...
    <div className="h-full max-h-[calc(100vh-340px)] flex flex-col overflow-hidden min-h-0">
      <div className="flex-shrink-0 h-40px flex justify-between bg-ppGridHeader text-white font-medium text-sm rounded-t-[5px] mx-20px">
        {columns.map((col) => (
          <div
            key={col.key}
            className={`flex justify-center items-center ${col.className} ${
      ...
      <div className="flex-1 overflow-y-auto min-h-0 custom-scrollbar ml-20px">
        {data.map((item, index) => (
          <div
            key={index}
            className={`h-40px flex justify-between text-gray text-base border-b-[1px] border-grayD9  ${selectedRowIndex === item.clsPassId ? 'bg-yellow' : 'bg-white'}`}
          ...
          >
            {columns.map((col) => {
              ...
              return (
                <div
                  key={col.key}
                  className={`flex justify-center items-center ${col.className} cursor-pointer ${col.key === 'clsPkgNm' ? 'truncate' : ''} ${
                    col.className?.includes('text-center')
                      ? 'flex justify-center items-center'
                      : col.className?.includes('text-right')
                        ? 'flex justify-end items-center'
      ...
      {/* 하단 합계 바 */}
      <div className="flex-shrink-0 overflow-y-auto mr-20px min-h-0 mx-20px mt-30px rounded-[5px] ">
        <div className="grid grid-cols-6 items-center">
          <div className="col-span-6 bg-grayCE text-black grid grid-cols-6 px-[10px] py-[10px]">
            <div className="px-[30px] border-r border-calGray">
              총 결제건수 <span className="float-right">{formatNumber(summary.totalPayCnt)}</span>
            </div>
            <div className="px-[30px] border-r border-calGray">
              총 결제금액 <span className="float-right">{formatNumber(summary.totalPayAmt)}</span>
            </div>
            <div className="px-[30px] border-r border-calGray">
              총 할인금액 <span className="float-right">{formatNumber(summary.totalDisAmt)}</span>
            </div>
            <div className="px-[30px] border-r border-calGray">
              총 환불건수 <span className="float-right">{formatNumber(summary.totalRefCnt)}</span>
            </div>
            <div className="px-[30px] border-r border-calGray">
              카드 결제건수 <span className="float-right">{formatNumber(summary.totalCardCnt)}</span>
            </div>
            <div className="px-[30px]">
              계좌 결제건수 <span className="float-right">{formatNumber(summary.totalCashCnt)}</span>
            </div>
          </div>
        </div>
      </div>
