...
const columns: {
    ...
    {key: 'paidAmt', title: '결제 금액', className: 'min-w-[120px] text-center'},
    {key: 'discountAmt', title: '기본할인금액', className: 'min-w-[120px] text-center'},
    {key: 'discountAmt2', title: '추가할인금액', className: 'min-w-[120px] text-center'},
    {key: 'totalCnt', title: '총 회차', className: 'min-w-[100px] text-center'},
    {key: 'remainCnt', title: '잔여 회차', className: 'min-w-[100px] text-center'},
    {
        key: 'expRate',
        title: '유효 기간',
                ...
                {data.map((item, index) => (
                    ...
                    >
                        {columns.map((col) => {
                            ...
                            return (
                                <div key={col.key}
                                     className={`flex justify-center items-center ${col.className} cursor-pointer ${
            ...
            <div className="flex-shrink-0 overflow-y-auto mr-20px min-h-0 mx-20px mt-30px rounded-default">
                <div className="grid grid-cols-6 items-center">
                    <div className="col-span-6 bg-whiteGray text-black grid grid-cols-6">
                        <div className="px-3 py-2 border-r border-lightGray">총 결제건수 <span
                            className="float-right">{formatNumber(summary.totalPayCnt)}</span></div>
                        <div className="px-3 py-2 border-r border-lightGray">총 결제금액 <span
                            className="float-right">{formatNumber(summary.totalPayAmt)}</span></div>
                        <div className="px-3 py-2 border-r border-lightGray">총 할인금액 <span
                            className="float-right">{formatNumber(summary.totalDisAmt)}</span></div>
                        <div className="px-3 py-2 border-r border-lightGray">총 환불건수 <span
                            className="float-right">{formatNumber(summary.totalRefCnt)}</span></div>
                        <div className="px-3 py-2 border-r border-lightGray">카드 결제건수 <span
                            className="float-right">{formatNumber(summary.totalCardCnt)}</span></div>
                        <div className="px-3 py-2">계좌 결제건수 <span
